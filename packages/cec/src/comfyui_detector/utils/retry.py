    ...
    if isinstance(error, urllib.error.HTTPError):
        ...
        if error.code == 403:
            ...
            try:
                ...
                if 'rate limit' in error_data.lower() or 'api rate limit' in error_data.lower():
                    return True
            except Exception:
                pass
...
def retry_on_rate_limit(config: Optional[RetryConfig] = None):
        ...
        try:
            ...
            if is_rate_limit_error(e):
                if attempt < config.max_retries:
                    ...
                    logger.error(f"Rate limit hit, max retries ({config.max_retries}) exceeded")
